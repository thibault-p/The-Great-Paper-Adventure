...
namespace TGPA.SpecialEffects
{
    ...
    /// Lot of hearts, kawaï !!!
    /// </summary>
    public class HeartWave : Particle
    {
        public HeartWave(Vector2 location,
            Vector2 trajectory,
            float size,
            int icon)
            : base(Color.White.R, Color.White.G, Color.White.B)
        {
            ...
            if (this.size > 1f)
                this.size = Math.Min(this.size, 1.1f); //Dirt hack to fix huge sizes
        ...
        public override void Draw(TGPASpriteBatch sprite, Texture2D tex)
        {
            ...
            Rectangle sRect = new Rectangle(flag * 64, 192, 64, 64);
            float frameAlpha;
